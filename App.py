...
def call_gemini(client, system, user, temperature=0.7, max_tokens=2048):
...
with st.sidebar:
    ...
    st.markdown("<div class='divider'></div>", unsafe_allow_html=True)
    st.markdown("<div style='font-size:.72rem;color:#475569;text-align:center'>Built for JEE/NEET aspirants 🚀<br>Powered by Gemini 2.5 Flash</div>", unsafe_allow_html=True)
...
if nav == "🏠  Home":
    ...
    st.markdown("<div class='page-sub'>Your personal AI-powered study companion for JEE & NEET success</div>", unsafe_allow_html=True)
    ...
    features = [
        ...
        ("⚡","badge-amber","QUICKQUIZ","Generate instant MCQs on any topic. Get immediate AI feedback on your reasoning not just right or wrong."),
    ]
    ...
    st.markdown("<div class='divider'></div>", unsafe_allow_html=True)
    st.info("👈 Pick a feature from the sidebar to get started. Make sure you have added your Gemini API key in secrets.", icon="💡")

elif nav == "🧑‍🏫  Mentor":
    st.markdown(f"<div class='page-title'>🧑‍🏫 AI Mentor</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='page-sub'>Ask any {subject} concept and get a clear analogy-rich explanation built for {exam}</div>", unsafe_allow_html=True)
    MENTOR_SYSTEM = f"""You are ExamZen Mentor, an elite {subject} teacher specialising in {exam} preparation.
Rules:
1. Always explain using at least ONE vivid real-world analogy.
2. Structure every reply:
   - Core Idea (1-2 sentences)
   - Real-World Analogy
   - The Science (equations if needed)
   - Common Exam Traps (1-3 bullets)
   - Memory Tip
3. Keep language friendly but precise.
4. Flag if a concept is HIGH-YIELD for {exam}.
5. Stay strictly on {subject} topics."""
    ...
    if send:
        if not client: api_missing()
            ...
            with st.spinner("Mentor is thinking..."):
                ...
                try:
                    reply=call_gemini(client,MENTOR_SYSTEM,full_prompt,temperature=0.65,max_tokens=1800)
    ...
    CORRECTIFY_SYSTEM = f"""You are ExamZen Correctify, a sharp diagnostic tutor for {exam} {subject}.
Given a question and a student's incorrect answer, produce:
1. What Went Wrong - identify the exact mistake in 1-2 sentences.
2. Root Cause - name the underlying misconception.
3. Correct Approach - step-by-step correct solution.
4. The Rule to Remember - a single memorable principle.
5. Similar Traps - 2 other question types using the same misconception.
6. Difficulty for {exam}: Easy / Medium / High
Be surgical, honest, and constructive."""
    col_a,col_b=st.columns(2,gap="large")
    with col_a:
        question_text=st.text_area("📋 The Question",placeholder="Paste the full question here...",height=160)
    with col_b:
        student_answer=st.text_area("🙋 Your Answer / Working",placeholder="What did you write or calculate?",height=160)
    if st.button("🔍 Diagnose My Mistake"):
        if not client: api_missing()
            ...
            with st.spinner("Diagnosing..."):
                try:
                    prompt=f"QUESTION:\n{question_text}\n\nSTUDENT'S ANSWER:\n{student_answer}"
                    diagnosis=call_gemini(client,CORRECTIFY_SYSTEM,prompt,temperature=0.4,max_tokens=1600)
    ...
    if not st.session_state.mistake_log:
        ...
        if st.button("🗑️ Clear Mistake Log"):
            st.session_state.mistake_log=[]
            st.rerun()
    ...
    TIMETABLE_SYSTEM=f"""You are ExamZen's study planner for {exam}. Create a focused 7-day revision schedule.
Output a valid Markdown table with these columns:
Day | Date | Topic | Subtopics to Cover | Resources | Goal / Target | Evening Revision
Rules:
- Weaker topics get more days and repetitions.
- Include interleaved revision (revisit Day 1 topics on Day 3/5/7).
- Add one practice test on Day 6 or 7.
- After the table add a 3-point Strategy Note section."""
    col1,col2=st.columns([3,2],gap="large")
    with col1:
        weak_topics=st.text_area("📌 Your Weak Topics",placeholder="e.g. Rotational Dynamics, Electrochemistry, Limits",height=120)
        daily_hours=st.slider("⏱️ Study hours per day",2,12,6)
    with col2:
        ...
        if not client: api_missing()
            ...
            with st.spinner("Building your personalised plan..."):
                try:
                    ...
                    plan=call_gemini(client,TIMETABLE_SYSTEM,prompt,temperature=0.5,max_tokens=2000)
                    st.success("Your plan is ready!",icon="🗓️")
                    st.markdown(plan)
                    st.download_button("⬇️ Download as Markdown",data=plan,file_name="examzen_timetable.md",mime="text/markdown")
                except Exception as e: st.error(f"API error: {e}",icon="🔴")

elif nav == "⚡  QuickQuiz":
    st.markdown("<div class='page-title'>⚡ QuickQuiz</div>",unsafe_allow_html=True)
    st.markdown("<div class='page-sub'>Generate sharp MCQs on any topic. Get instant AI feedback on your reasoning.</div>",unsafe_allow_html=True)
...
Return ONLY a JSON array with no explanation outside it. Schema:
[{{"q":"question text","options":{{"A":"...","B":"...","C":"...","D":"..."}},"answer":"A","explanation":"Why correct and others wrong.","difficulty":"Easy|Medium|Hard","topic_tag":"subtopic"}}]
Make distractors plausible. Align with {exam} pattern."""
    FEEDBACK_SYSTEM=f"""You are a kind but precise {exam} coach for {subject}.
Analyse the student's MCQ answer and give:
1. Correct or Incorrect
2. Why the correct answer is right
3. Why the wrong choice is a classic trap (if wrong)
...
Keep it under 150 words."""
        ...
        if not client: api_missing()
            ...
            with st.spinner("Generating questions..."):
                try:
                    ...
                    raw=call_gemini(client,system,quiz_topic,temperature=0.75,max_tokens=2500)
    ...
    if qs and "questions" in qs:
        ...
        for i,q in enumerate(qs["questions"]):
            ...
            st.markdown(f"<div style='font-size:.72rem;font-weight:700;color:{diff_color};text-transform:uppercase;letter-spacing:.07em'>{q.get('difficulty','?')} · {q.get('topic_tag','')}</div>",unsafe_allow_html=True)
            ...
            if st.button(f"Check Answer",key=f"check_{i}"):
                ...
                with st.spinner("Analysing..."):
                    try:
                        ...
                        fb=call_gemini(client,FEEDBACK_SYSTEM,fb_prompt,temperature=0.4,max_tokens=400)
                        qs["feedback"][i]=fb
    ...
    if st.session_state.mistake_log:
        ...
        st.info("Use Correctify to diagnose mistakes — they will appear here as stats.",icon="🔬")
    st.markdown("<div class='divider'></div>",unsafe_allow_html=True)
